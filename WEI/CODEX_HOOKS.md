...
지금은 실제 프로젝트가 상위 저장소의 `WEI/` 폴더 아래에 들어 있습니다.

그래서 Codex를 상위 저장소 루트에서 열면, 루트 `.codex` 설정이 `WEI/logs/user-prompts.jsonl`로 기록하도록 맞춰져 있습니다.

반대로 `WEI/` 자체를 작업 루트로 열면 이 문서에 있는 `.codex` 설정이 그대로 동작합니다.
...
`WEI/`를 작업 루트로 열면 로그는 `logs/user-prompts.jsonl`에 저장됩니다.

상위 저장소 루트에서 열면 로그는 `WEI/logs/user-prompts.jsonl`에 저장됩니다.
